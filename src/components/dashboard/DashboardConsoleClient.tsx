...
interface DashboardConsoleClientProps {
  slug: string;
  serverHasWriteAccess: boolean;
}

export function DashboardConsoleClient({ slug, serverHasWriteAccess }: DashboardConsoleClientProps) {
  ...
  // Unified reactive state that controls all mutation fields across the showroom dashboard
  const [isEditModeActive, setIsEditModeActive] = useState(serverHasWriteAccess);
  ...
  const handleEditRedirect = (productId: string) => {
    router.push(`/dashboard/inventory/manage?id=${productId}`);
  };
  ...
  // Make sure to sync initial edit state with active server authorization level
  useEffect(() => {
    setIsEditModeActive(serverHasWriteAccess);
  }, [serverHasWriteAccess]);
  ...
  const handleStepAction = (id: string) => {
    if (id === "verification") {
      if (!isEditModeActive && serverHasWriteAccess) setIsEditModeActive(true);
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] font-sans text-[#F9F6EE] selection:bg-[#D4CAA3] selection:text-black antialiased">
      ...
      <ShowroomHeader 
        ...
        hasWriteAccess={serverHasWriteAccess} 
      ...
      <div data-section="vault">
        <VerificationVault hasWriteAccess={serverHasWriteAccess} isEditMode={isEditModeActive} />
      </div>
      ...
      {showOnboarding ? (
        ...
        <ProductGrid 
          ...
          onEditInit={(product) => handleEditRedirect(product.id)} 
        />
      )}
